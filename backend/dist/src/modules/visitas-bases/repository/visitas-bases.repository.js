...
Object.defineProperty(exports, "VisitasBasesService", {
    enumerable: true,
    get: function() {
        return VisitasBasesService;
    }
});
...
let VisitasBasesService = class VisitasBasesService {
    ...
    async findOne(id) {
        return this.prisma.visitasBases.findUnique({
            where: {
                id
            }
        });
    ...
    async create(data) {
        if (data.base) {
            ...
            if (!base) {
                throw new _common.BadRequestException('Base não encontrada.');
            }
            ...
            if (exist) {
                throw new _common.BadRequestException('Visita já cadastrada para esta base nessa data.');
            }
        }
        if (data.outro_motivo) {
            ...
            if (exist) {
                throw new _common.BadRequestException('Visita já cadastrada para este motivo nessa data.');
            }
        }
    ...
    async update(id, data) {
        ...
        if (!visitaExistente) {
            throw new _common.BadRequestException('Visita não encontrada.');
        }
        if (data.base) {
            const base = await this.prisma.base.findUnique({
                where: {
    ...
    async remove(id) {
        ...
        return this.prisma.visitasBases.delete({
            where: {
                id
            }
        });
...
VisitasBasesService = _ts_decorate([
